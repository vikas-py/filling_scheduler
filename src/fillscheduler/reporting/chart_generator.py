...
def generate_utilization_chart_image(
    ...
    fig.add_trace(go.Scatter(
        ...
        name='Filling',
        fill='tozeroy',
        line=dict(color='#1976d2', width=0),
        fillcolor='rgba(25, 118, 210, 0.7)',
    ))
    
    fig.add_trace(go.Scatter(
        x=utilization_by_hour['hour'],
        y=[f + c for f, c in zip(utilization_by_hour['filling'], utilization_by_hour['cleaning'])],
        mode='lines',
        name='Cleaning',
        fill='tonexty',
...
def generate_filler_breakdown_data(
    ...
    for filler_id in range(1, num_fillers + 1):
        ...
        filler_stats.append({
            ...
            'lots_processed': len(set(a['lot_id'] for a in filler_activities if a.get('kind') == 'FILL')),
        })
